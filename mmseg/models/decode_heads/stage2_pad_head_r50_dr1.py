...
class Stage2PADHead_r50_dr1(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels,
                 ...
                 **kwargs):
        super(Stage2PADHead_r50_dr1, self).__init__(**kwargs)
        ...
        #                   channels * num_classes + \
        #                   num_classes
        self.dr1_conv = nn.Sequential(
            nn.Conv2d((2+num_bases), self.dyn_ch, 1, padding=0, bias=False),
            nn.Conv2d(self.dyn_ch, self.dyn_ch, 1, padding=0, bias=False),
            nn.Conv2d(self.dyn_ch, self.pad_out_channel, 1, padding=0, bias=False),
        )
        nn.init.constant_(self.dr1_conv[0].weight, 1.0)
        nn.init.constant_(self.dr1_conv[1].weight, 1.0)
        nn.init.constant_(self.dr1_conv[2].weight, 1.0)
    ...
    def subnetworks_forward_fast(self, inputs, weights, biases, n_subnets):
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            if i == 0:
                x = self.padconv_dr1(x, w, b, cin=2 + num_bases,
                                     cout=self.dyn_ch, relu=True,
                                     conv_weight=self.dr1_conv[i].weight)
            if i == 1:
                x = self.padconv_dr1(x, w, b, cin=self.dyn_ch,
                                     cout=self.dyn_ch, relu=True,
                                     conv_weight=self.dr1_conv[i].weight)
            if i == 2:
                x = self.padconv_dr1(x, w, b, cin=self.dyn_ch,
                                     cout=self.pad_out_channel, relu=True,
                                     conv_weight=self.dr1_conv[i].weight)
        return x
    ...
    def padconv_dr1(self, input, w, b, cin, cout, relu, conv_weight):
        input = input.repeat(1, cout, 1, 1)
        x = input * w
        x = F.conv2d(
            x, conv_weight, stride=1, padding=0,
            groups=cout
        )
        x = x + b
        if relu:

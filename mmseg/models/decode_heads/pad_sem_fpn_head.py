...
class PADSEMFPNHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        super(PADSEMFPNHead, self).__init__(
    ...
    def forward(self, inputs):
        ...
        x_out = self.scale_heads[0](features[0])
        for i in range(1, len(self.feature_strides)):
            # non inplace
            x_out = x_out + resize(
                self.scale_heads[i](features[i]),
                size=x_out.shape[2:],
                mode='bilinear',
                align_corners=self.align_corners)

        assert self.use_low_level_info
        x_cat = self.bottleneck(x_out)

...
class LastHopeHead_fast(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels,
                 ...
                 **kwargs):
        super(LastHopeHead_fast, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        output = self.interpolate_fast(dyn_filters, output, self.cat_norm)
        if plot:
            outputs = []
    ...
    def get_subnetworks_params(self, attns, num_bases=0, channels=16):
        ...
        w0, b0, w1, b1, w2, b2 = torch.split_with_sizes(attns, [
            ...
            (channels + num_bases) * self.pad_out_channel, self.pad_out_channel
        ], dim=1)
        ...
        w2 = w2.reshape(n_inst * self.pad_out_channel, channels + num_bases, 1, 1)
    ...
    def subnetworks_forward(self, inputs, weights, biases, n_subnets, x_cat=None, norm=None, B=None, H=None, W=None):
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            if i == 2 and x_cat is not None:
                f = self.upsample_f
                x = x.reshape(B, H, W, self.dyn_ch, f, f).permute(0, 3, 1, 4, 2, 5)
                x = x.reshape(B, self.dyn_ch, H * f, W * f)

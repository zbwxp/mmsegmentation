...
class Stage2PADHead_r50_ASPP_ver(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels,
                 ...
                 **kwargs):
        super(Stage2PADHead_r50_ASPP_ver, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        aspp_half = torch.cat((aspp_outs[0], aspp_outs[2], aspp_outs[3], aspp_outs[4]), dim=1)
        output = torch.cat((self.bottleneck(aspp_half), aspp_outs[1]), dim=1)

        # x1 = self.stage5_head(x)
        # x2 = self.global_pool(x1).expand_as(x1)
        #

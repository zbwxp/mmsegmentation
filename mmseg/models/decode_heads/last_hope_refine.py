...
class DynHead(nn.Module):
    def __init__(self,
        ...
        if use_low_level_info:
            num_bases = mask_ch
        num_out_channel = (2) * channels + \
                          channels + \
                          (channels) * num_bases + \
                          num_bases + \
                          (num_bases + num_bases) * num_classes + \
                          num_classes
...
class LastHopeHead_refine(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels,
                 ...
                 **kwargs):
        super(LastHopeHead_refine, self).__init__(**kwargs)
        ...
        _, norm = build_norm_layer(self.norm_cfg, self.mask_ch + self.mask_ch)
    ...
    def interpolate(self, x, x_cat=None, norm=None):
        ...
        coord = coord.reshape(B_coord, ch_coord, H, f, W, f).permute(0, 2, 4, 1, 3, 5).reshape(1,
                                                                                               ...
                                                                                               f, f)
        output = self.subnetworks_forward(coord, weights, biases, B * H * W, x_cat, norm, B, H, W)
    ...
    def get_subnetworks_params(self, attns, num_bases=0, channels=16):
        ...
        w0, b0, w1, b1, w2, b2 = torch.split_with_sizes(attns, [
            (2) * channels, channels,
            channels * num_bases, num_bases,
            (num_bases + num_bases) * self.pad_out_channel, self.pad_out_channel
        ], dim=1)

        # out_channels x in_channels x 1 x 1
        w0 = w0.reshape(n_inst * channels, 2, 1, 1)
        b0 = b0.reshape(n_inst * channels)
        w1 = w1.reshape(n_inst * num_bases, channels, 1, 1)
        b1 = b1.reshape(n_inst * num_bases)
        w2 = w2.reshape(n_inst * self.pad_out_channel, num_bases + num_bases, 1, 1)
        b2 = b2.reshape(n_inst * self.pad_out_channel)

        return [w0, w1, w2], [b0, b1, b2]

    def subnetworks_forward(self, inputs, weights, biases, n_subnets, x_cat=None, norm=None, B=None, H=None, W=None):
        assert inputs.dim() == 4
        n_layer = len(weights)
        x = inputs
        # NOTE: x has to be treated as min_batch size 1
        for i, (w, b) in enumerate(zip(weights, biases)):
            if i == 2 and x_cat is not None:
                f = self.upsample_f
                x = x.reshape(B, H, W, self.mask_ch, f, f).permute(0, 3, 1, 4, 2, 5)
                x = x.reshape(B, self.mask_ch, H * f, W * f)
                x = torch.cat((x, x_cat), 1)
                x = norm(x)
                B_coord, ch_coord, H_coord, W_coord = x.size()
                x = x.reshape(B_coord, ch_coord, H, f, W, f).permute(0, 2, 4, 1, 3, 5).reshape(1,
                                                                                               B_coord * H * W * ch_coord,
                                                                                               f, f)

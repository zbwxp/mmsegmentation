...
class DynHead(nn.Module):
    def __init__(self,
        ...
        if use_low_level_info:
            num_bases = mask_ch
        num_out_channel = ((2 + num_bases) * channels) * 9 + \
                          channels + \
                          channels * channels + \
                          channels + \
...
class DeepPadHead3x3(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels,
                 ...
                 **kwargs):
        super(DeepPadHead3x3, self).__init__(**kwargs)
    ...
    def get_subnetworks_params(self, attns, num_bases=0, channels=16):
        ...
        w0, b0, w1, b1, w2, b2 = torch.split_with_sizes(attns, [
            9 * ((2 + num_bases) * channels), channels,
            channels * channels, channels,
            channels * self.pad_out_channel, self.pad_out_channel
        ], dim=1)

        # out_channels x in_channels x 1 x 1
        w0 = w0.reshape(n_inst * channels, 2 + num_bases, 3, 3)

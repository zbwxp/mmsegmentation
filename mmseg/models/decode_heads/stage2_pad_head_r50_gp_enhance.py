...
class Stage2PADHead_r50_GPEnhance(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels,
                 ...
                 **kwargs):
        super(Stage2PADHead_r50_GPEnhance, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        B, ch, H, W = x.size()
        x1 = self.global_pool(x).expand_as(x)
        x_stack = torch.stack((x, x1), dim=2)
        x_stack = x_stack.view(B, 2 * ch, H, W)
        x = self.gp_enhance(x_stack)
        output = x
